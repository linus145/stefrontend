...
export function LeftSidebar({ isCollapsed, onToggle, activeSection, onSectionChange }: LeftSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "h-screen fixed left-0 top-0 bg-sidebar border-r border-border flex flex-col justify-between py-6 z-40 transition-all duration-300 ease-in-out",
    ...
    >
      ...
      <button
        onClick={onToggle}
        className="absolute -right-3 top-20 flex h-6 w-6 items-center justify-center rounded-full bg-surface border border-border text-muted-foreground hover:text-primary transition-all z-50 shadow-sm hover:scale-110 active:scale-95"
      >
      ...
      <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
        {/* Brand */}
        <div className={cn("px-6 flex items-center gap-3 transition-all", isCollapsed ? "px-5" : "px-6")}>
      ...
      <div className={cn("px-4 space-y-6 transition-all", isCollapsed ? "px-4" : "px-4")}>
        <div className="space-y-3 pb-2">
          <button className={cn(
            ...
            {!isCollapsed && <span className="animate-in fade-in duration-300">Raise Capital</span>}
          </button>
