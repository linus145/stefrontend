  ...
  return (
    <div className="space-y-4 pt-2">
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-slate-50/50 dark:bg-[#1c1d30]/30 p-3.5 border border-slate-200/60 dark:border-slate-800/80 rounded-sm">
          ...
          <div className="md:col-span-3 space-y-1">
            ...
            <input
              ...
              className="w-full h-9 px-3 text-xs bg-slate-50 dark:bg-[#1c1d30] border border-slate-200 dark:border-slate-850 rounded-sm font-semibold focus:outline-none focus:border-[#0a66c2]"
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  onAddStep();
          ...
          <div className="md:col-span-2 space-y-1 relative">
            ...
            <div className="relative">
              <button
                ...
                className="w-full h-9 px-3 text-left text-xs bg-slate-50 dark:bg-[#1c1d30] border border-slate-200 dark:border-slate-850 rounded-sm font-bold focus:outline-none focus:border-[#0a66c2] flex items-center justify-between cursor-pointer"
              >
              ...
              {isPickerOpen && (
                <div 
                  ref={pickerRef}
                  className="absolute right-0 bottom-full md:bottom-auto md:top-full z-50 mt-1 p-3 bg-white dark:bg-[#121320] border border-slate-200 dark:border-slate-800 rounded-sm shadow-lg flex gap-1 items-center justify-center min-w-[250px] animate-in fade-in slide-in-from-top-1 duration-150"
                >
                  {/* Hour Select */}
                  <select
                    value={hour12}
                    ...
                    className="w-[22%] h-8 text-center text-xs bg-slate-50 dark:bg-[#1c1d30] border border-slate-200 dark:border-slate-800 rounded-sm font-bold focus:outline-none focus:border-[#0a66c2] cursor-pointer"
                  >
                  ...
                  <select
                    ...
                    className="w-[28%] h-8 text-center text-xs bg-slate-50 dark:bg-[#1c1d30] border border-slate-200 dark:border-slate-800 rounded-sm font-bold focus:outline-none focus:border-[#0a66c2] cursor-pointer"
                  >
          ...
          {editingStepIndex !== null ? (
            <div className="md:col-span-2 flex gap-1.5 w-full">
              <Button
                ...
                className="flex-1 bg-[#0a66c2] hover:bg-[#084e96] text-white font-extrabold text-[11px] h-9 rounded-sm flex items-center justify-center cursor-pointer transition-colors"
              >
                Save
              </Button>
              <Button
                ...
                className="flex-1 bg-[#0a66c2]/80 hover:bg-[#084e96] text-white font-extrabold text-[11px] h-9 rounded-sm flex items-center justify-center cursor-pointer transition-colors"
              >
                Cancel
              </Button>
            </div>
            ...
            <div className="md:col-span-2 flex w-full">
              <Button
                ...
                disabled={!enabled}
                onClick={onAddStep}
                className="w-full bg-[#0a66c2] hover:bg-[#084e96] text-white font-extrabold text-xs h-9 rounded-sm flex items-center justify-center cursor-pointer transition-colors"
              >
                Add Step
              </Button>
            </div>
          ...
          {commandSteps.length > 0 ? (
            commandSteps.map((step, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center justify-between gap-3 p-3 bg-slate-50/70 dark:bg-[#1c1d30]/40 border border-slate-200/80 dark:border-slate-800/80 rounded-sm shadow-2xs hover:border-[#0a66c2]/30 dark:hover:border-[#0a66c2]/30 transition-all duration-200"
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 flex-1 min-w-0">
                    {/* Task Badge */}
                    <div className="shrink-0 flex items-center gap-1.5">
                      <Badge className="bg-[#0a66c2]/10 hover:bg-[#0a66c2]/15 text-[#0a66c2] border-none font-bold text-[9px] px-2 py-0.5 rounded-sm uppercase">
                        {matchedTask ? matchedTask.label : step.task}
                      </Badge>
                      <Badge className="bg-emerald-500/10 text-emerald-600 border-none font-bold text-[9px] px-2 py-0.5 rounded-sm uppercase">
                    ...
                    {/* Trigger Time Badge */}
                    <div className="shrink-0 flex items-center gap-1 text-[10px] text-slate-450 dark:text-slate-500 font-bold bg-slate-100 dark:bg-[#151624] px-2 py-0.5 rounded-sm">
                  ...
                  <div className="shrink-0 flex items-center gap-1">
                    <button
                      ...
                      className="text-[10px] font-extrabold text-[#0a66c2] hover:text-[#084e96] hover:bg-[#0a66c2]/10 px-2 py-1.5 rounded-sm cursor-pointer transition-all"
                    >
                      Edit
                    </button>
                    <button
                      ...
                      className="text-[10px] font-bold text-red-500 hover:text-red-750 hover:bg-red-500/10 p-1.5 rounded-sm cursor-pointer transition-all shrink-0"
                    >
                      ✕
                    </button>
                  </div>
            ...
            <div className="py-6 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-sm">
